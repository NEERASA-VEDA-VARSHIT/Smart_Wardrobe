...
const Laundry = () => {
  ...
  const fetchLaundryItems = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:8000/api' : 'https://smart-wardrobe-backend.vercel.app/api')}/laundry/${user._id}`, {
        credentials: 'include'
      });
  ...
  const markAsWashed = async (clothingId) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:8000/api' : 'https://smart-wardrobe-backend.vercel.app/api')}/laundry/return/${clothingId}`, {
        method: 'POST',
        headers: {
  ...
  const removeFromLaundry = async (clothingId) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:8000/api' : 'https://smart-wardrobe-backend.vercel.app/api')}/laundry/remove/${clothingId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
