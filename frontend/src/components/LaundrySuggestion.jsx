...
const LaundrySuggestion = ({ onItemMovedToLaundry, onDismiss }) => {
  ...
  const fetchSuggestions = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:8000/api' : 'https://smart-wardrobe-backend.vercel.app/api')}/laundry-suggestions/${user._id}`, {
        credentials: 'include'
      });
  ...
  const moveToLaundry = async (clothingId, itemType) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:8000/api' : 'https://smart-wardrobe-backend.vercel.app/api')}/laundry/add/${clothingId}`, {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        ...
        await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:8000/api' : 'https://smart-wardrobe-backend.vercel.app/api')}/laundry-suggestions/learn`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const updateWashPreference = async (clothingId, preference) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:8000/api' : 'https://smart-wardrobe-backend.vercel.app/api')}/laundry-suggestions/wash-preference/${clothingId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
