  ...
  const fetchWeatherRecommendations = async (lat, lon) => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:8000/api' : 'https://smart-wardrobe-backend.vercel.app/api')}/weather-recommendations/${user._id}?lat=${lat}&lon=${lon}&includeForecast=true`,
        {
          credentials: 'include'
        }
      );
